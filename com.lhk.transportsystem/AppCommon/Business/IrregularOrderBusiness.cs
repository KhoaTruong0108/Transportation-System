...
{
    public class IrregularOrderBusiness : GenericBusiness<IrregularOrder>, IOrderBusiness<IrregularOrder>
    {
        ...
        public IrregularOrderBusiness(User currentUser = null)
            : base(currentUser)
        {
            UseAuditTrail = true;
        }
        ...
        public virtual string ChangeStatus(string id, string deliveryStatus)
        {
            ...
            {
                AppLogger.logError(this.ToString(), string.Format("Error occurs while updating order status.", id), ex);
                Repository.Rollback();
        ...
        public virtual string Insert(IrregularOrder order, Collection<OrderItem> orderItems)
        {
            string result = string.Empty;
            AppLogger.logInfo(this.ToString(), "Begin inserting new order.");
            try
            {
            ...
            AppLogger.logInfo(this.ToString(), "Finish inserting new order.");
            return result;
        }

        public virtual string Update(IrregularOrder order, Collection<OrderItem> orderItems)
        {
            string result = string.Empty;
        ...
        protected override void LogAuditTrail(IrregularOrder order, string action)
        {
            if (CurrentUser != null)
            {
